...
namespace SAM.Picker
{
    internal partial class GamePicker : Form
    {
        ...
        private void DoDownloadList(object? sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                bytes = _HttpClient.GetByteArrayAsync("https://gib.me/sam/games.xml").GetAwaiter().GetResult();
            }
            catch (Exception ex)
            {
                throw new Exception("Failed to download game list", ex);
            }
        ...
        private void OnDownloadList(object? sender, RunWorkerCompletedEventArgs e)
        {
            if (e.Error != null || e.Cancelled == true)
            {
                ...
                MessageBox.Show(e.Error?.ToString() ?? "Unknown error", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        private void OnGameListViewSearchForVirtualItem(object sender, SearchForVirtualItemEventArgs e)
        {
            ...
            {
                predicate = gi => gi.Name != null && text != null && gi.Name.StartsWith(text, StringComparison.CurrentCultureIgnoreCase);
            }
            /*else
            {
        ...
        private void DoDownloadLogo(object? sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                var data = _HttpClient.GetByteArrayAsync(info.ImageUrl).GetAwaiter().GetResult();
