...
namespace SAM.Game
{
    internal partial class Manager : Form
    {
        ...
        private bool _IsDownloadingIcon = false;
        ...
        private void OnIconDownload(Stats.AchievementInfo info, byte[]? data)
        {
            if (data != null)
            {
                Bitmap? bitmap;
        ...
        private async void DownloadNextIcon()
        {
            ...
            if (this._IsDownloadingIcon == true)
            {
                return;
            }
            ...
            try
            {
                ...
                var data = await this._HttpClient.GetByteArrayAsync(url);
                this.OnIconDownload(info, data);
            }
            catch
            {
                this.OnIconDownload(info, null);
            }
        }
        ...
        {
            if (sender is ToolStripButton button && button.Checked == true)
            {
